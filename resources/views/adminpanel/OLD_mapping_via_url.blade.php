                                    ...
                                    @foreach($csv_headers as $key => $csv_header)
                                        <div class="d-flex mb-5 justify-content-between align-items-center">
                                            <div class="w-25">
                                                ...
                                                <input type="hidden" value="{{$csv_header}}" name="mapped_csv_header[]">
                                            </div>
                                            <div class="w-75">
                                                <select name="shopify_fields[]" class="form-control shopify-select" aria-invalid="false">
                                                    <option value="" disabled="">Select shopify product attribute...</option><option value="ignore" selected>Ignore</option><option value="description">Description (Body HTML)</option><option value="collection">Collection</option><option value="handle">Handle</option><option value="image">Image</option><option value="multiple_images">Multiple Images</option><option value="metafields_global_title_tag">Metafields global title tag</option><option value="metafields_global_descriptio_tag">Metafields global description tag</option><option value="metafield">Metafield</option><option value="option_name_1">Option name 1</option><option value="option_name_2">Option name 2</option><option value="option_name_3">Option name 3</option><option value="product_type">Product type</option><option value="published_scope">Published scope</option><option value="published">Published</option><option value="tags">Tags</option><option value="template_suffix">Template suffix</option><option value="title">Title</option><option value="vendor">Vendor</option><option value="variant_barcode">- Variant Barcode</option><option value="variant_compare_at_price">- Variant Compare at price</option><option value="variant_fulfillment_service">- Variant Fulfillment service</option><option value="variant_inventory_management">- Variant Inventory management</option><option value="variant_inventory_policy">- Variant Inventory policy</option><option value="variant_inventory_quantity">- Variant Inventory quantity</option><option value="variant_cost_per_item">- Variant - Cost per item</option><option value="variant_metafield">- Variant Metafield</option><option value="variant_option1_value">- Variant Option 1 Value</option><option value="variant_option2_value">- Variant Option 2 Value</option><option value="variant_option3_value">- Variant Option 3 Value</option><option value="variant_position">- Variant Position</option><option value="variant_price">- Variant Price</option><option value="variant_requires_shipping">- Variant Requires shipping</option><option value="variant_sku">- Variant Sku</option><option value="variant_taxable">- Variant Taxable</option><option value="variant_title">- Variant Title</option><option value="variant_weight">- Variant Weight</option><option value="variant_weight_unit">- Variant Weight unit</option>
                                                </select>
                                            </div>
                                        </div>
                                    @endforeach
...
@section('js_after')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // var selected_shopify_value = $('.shopify-select option[value="ignore"]').attr("selected", "selected");
            // var shopify_select_array =  $(".shopify-select :selected").map(function(i, el) {
            //     return $(el).val();
            // }).get();
            // // console.log(shopify_select_array)
            // for (var i = 0; i < shopify_select_array.length; i++){
            //     // var option = select.options[i];
            //     // now have option.text, option.value
            //     // console.log(shopify_select_array[i]);
            //     if(shopify_select_array[i] == 'ignore'){
            //         // console.log(true)
            //     }
            // }
            ...
            $("#csv-mapped-field-form").submit(function(e){
                ...
                console.log(check_variant_sku && check_title)
                if((check_variant_sku > -1) && (check_title > -1) ){
